...
\begin{equation}
W_\text{замк.}(s) = \frac{s + 3}{2s^3 + 2s^2 + (3 + K)s + 3K}
\end{equation} \par
...
\begin{minipage}[t]{0.5\textwidth}
	\begin{figure} [H]
		...
		\addplot[only marks, mark = x, mark size = 5pt] coordinates {(-0.2298, 1.2141) (-0.2298, -1.2141) (-0.5404, 0)}; 
		\addplot[only marks, mark = o, mark size = 5pt] coordinates {(-3, 0)};
		\legend{полюса, нули};
		\end{axis}
		\end{tikzpicture}
